...
const traceForOtherChars = (grid, word, result) => {
  const { r, c } = result[0];
  ...
  for (let i=1; i< word.length; i++) {
    ...
    // traverse right
    if (currentChar ===  grid[r + i][c]) {
      rigthRes.push({ r: r + i, c });
    }
    
    if (currentChar ===  grid[r][c + i]) {
      downRes.push({ r: r, c: c + i });
    }
  ...
  for (let i=0; i < grid.length; i++) {
    ...
    for (let j=0; j < grid[i].length; j++) {
      if (grid[i][j] === word.charAt(0)) {
        // start tracing
        result.push({ r: i, c: j});
        traceForOtherChars(grid, word, result);
      }
...
console.log(findString(grid1, word1))